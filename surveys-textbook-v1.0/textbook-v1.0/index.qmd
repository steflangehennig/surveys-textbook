...
You only need to use `install.packages()` once unless you update your `R` with a newer version. Each time you close or restart `RStudio`, however, you must explicitly load in each package using the `library()` function. 
